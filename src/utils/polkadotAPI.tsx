...
// Add stake
export const addStake = async (account: string, amount: bigint) => {
  if (!account || !amount) {
  ...
  try {
    ...
    await customExtrinsic.signAndSend(
      ...
      ({ events = [], status }) => {
        ...
        if (status.isInBlock) {
          console.log("Included at block hash:", status.asInBlock.toHuman());
...
// Remove stake
export const removeStake = async (account: string, amount: bigint) => {
  console.log("account: ", account);
  console.log("amount: ", amount);
  if (!account || !amount) {
